...
class Eval:
    ...
    def solve_permutaiton(self):
        res, val = [*range(1, K+1)], 0
        list = [*permutations(range(1, K+1))]
        if K == 5:
            list = list[::6]
        for pmt in list:
            score = self.calc_score(self._solve_connect([*pmt]))
            if val < score:
                res = [*pmt]
        self.connects = self._solve_connect(res)
        return
...
def main():
    ...
    """
    move = Move(Grid())
    move.solve_sparse(40)
    eval = Eval(move.grid.blocks, move.lim_connect)
    eval.solve_permutaiton()
    Result(move.moves, eval.connects)
    return
