...
def main():
    ...
    id = 0
    ans = []
    while sum(sum(wi) for wi in hook.will_make) > 0:
        c_id, s_id, nxt = 0, 0, (0, 0)
        if hook.bfo[1] in prio.c_route:
            c_id = prio.c_route.index(hook.bfo[1])
        if hook.bfo[0] in prio.s_route:
            s_id = prio.s_route.index(hook.bfo[0])

        for ci in prio.c_route[c_id:]:
